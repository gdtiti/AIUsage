...
<p align="center">
  ...
  <img alt="版本" src="https://img.shields.io/badge/version-0.2.9-22c55e?style=flat-square">
...
| `Claude Code 代理` | 协议转换层，暴露 Claude 兼容 API 同时转发到 OpenAI 兼容后端，支持多配置管理 |
...
<table>
  ...
  <tr>
    <td colspan="2" align="center"><strong>菜单栏快览</strong></td>
  </tr>
</table>
...
- ✅ 完整的 Claude Messages API 支持 (`/v1/messages`)
- ✅ Token 计数端点 (`/v1/messages/count_tokens`)
- ✅ 流式 SSE 响应
- ✅ Tool use / 函数调用
- ✅ 图片支持（base64 编码）
- ✅ 模型名称规范化（`claude-sonnet-4.5` → `sonnet` → `gpt-4o`）
- ✅ 自定义请求头（用于追踪和认证）
- ✅ 可配置的上游服务商（OpenAI、Azure、Ollama 等）
