...
`credentialSourceFilePath(for:)` 从 `.authFile` 凭据中提取磁盘路径写入 summary，使凭据分支和自动扫描分支在 `identityKey` 上汇合为同一个 key，避免同一 workspace 出现两条记录。
...
1. accountId 匹配时 → **必须双侧 sourceIdentifier 非 nil 且匹配**（防止 nil == nil 误匹配）
...
1. **非 authFile 的 Codex 凭据**（如手动 token 登录）：无法产生 sourceFilePath，引擎 identityKey 会落到 `:cred:` 或 `:generic:`。当前 Codex 产品形态以 auth 文件为主，风险低。
2. **`CODEX_AUTH_FILE` 环境变量**：早退分支的 path 字面量可能与扫描发现的不完全一致，依赖下游 `normalizedAuthFilePath` 归一化解。
